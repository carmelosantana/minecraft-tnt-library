...
- [x] Successful main Actions run is recorded before tagging. → **v0.2.1:** main run `32851106059` **completed / success** on commit `ecba6df` (release-prep). **v0.2.0:** main run `32667062564` on `e457ac3`. (v0.1.1 was `32657520501` on `f1d7f5f`; v0.1.0 was `32654660644` on `8a9a6ed`.)
- [x] Workflow permissions contain no broader access than the documented contract. → `permissions: contents: write` only.

## 9. Release

- [x] Semantic version matches the POM, plugin metadata, and `v<version>` tag. → **v0.2.1** in `pom.xml`; `plugin.yml` `version: '${project.version}'` → embedded `0.2.1`; annotated tag `v0.2.1` on commit `ecba6df`. (Prior: `v0.2.0` on `e457ac3`, `v0.1.1` on `f1d7f5f`, `v0.1.0` on `8a9a6ed`.)
- [x] Successful tag Actions run and GitHub release are recorded. → **v0.2.1** tag run `32851208914` **completed / success** (incl. PackSquash pack + tagged-asset upload); release `v0.2.1` published (not draft/prerelease) — https://github.com/carmelosantana/minecraft-tnt-library/releases/tag/v0.2.1. (v0.2.0 tag run was `32667121555`.)
- [x] Release contains exactly one updater-matching JAR plus `SHA256SUMS.txt` and no `original-*` JAR. → **v0.2.1** assets: `tnt-library-0.2.1.jar` + `SHA256SUMS.txt` (plus the resource-pack sidecars `tnt-library-pack-0.2.1.zip` + `.zip.sha1`, which are not JARs and do not match the updater regex); `original-*` count 0.
- [x] Downloaded release assets pass `sha256sum --check SHA256SUMS.txt`. → **v0.2.1:** `tnt-library-0.2.1.jar: OK`. **Delivery integrity also verified:** the JAR's baked `pack.sha1` (`4dade8a74ad4c1b660f14ab91abfa3c01c203f3f`) exactly equals the published `tnt-library-pack-0.2.1.zip` SHA-1, and `pack.url` points to the v0.2.1 asset — so a Java client fetches the exact published pack and its hash check passes. (The v0.2.1 fixes changed no Java-pack content; the differing hash from v0.2.0 reflects the version-named zip, and baked==published is what guarantees delivery.)
