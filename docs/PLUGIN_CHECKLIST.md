...
- [x] Successful main Actions run is recorded before tagging. → **v0.2.0:** main run `32667062564` **completed / success** on commit `e457ac3` (the tagged Phase-2 merge). (v0.1.1 was `32657520501` on `f1d7f5f`; v0.1.0 was `32654660644` on `8a9a6ed`.)
- [x] Workflow permissions contain no broader access than the documented contract. → `permissions: contents: write` only.

## 9. Release

- [x] Semantic version matches the POM, plugin metadata, and `v<version>` tag. → **v0.2.0** in `pom.xml`; `plugin.yml` `version: '${project.version}'` → embedded `0.2.0` (verified in the running stack: "Loading server plugin TNTLibrary v0.2.0"); annotated tag `v0.2.0` on commit `e457ac3`. (Prior: `v0.1.1` on `f1d7f5f`, `v0.1.0` on `8a9a6ed`.)
- [x] Successful tag Actions run and GitHub release are recorded. → tag run `32667121555` **completed / success** (incl. the PackSquash pack + tagged-asset upload steps); release `v0.2.0` published (not draft/prerelease) — https://github.com/carmelosantana/minecraft-tnt-library/releases/tag/v0.2.0
- [x] Release contains exactly one updater-matching JAR plus `SHA256SUMS.txt` and no `original-*` JAR. → assets: `tnt-library-0.2.0.jar` + `SHA256SUMS.txt` (plus the resource-pack sidecars `tnt-library-pack-0.2.0.zip` + `.zip.sha1`, which are not JARs and do not match the updater regex); `original-*` count 0.
- [x] Downloaded release assets pass `sha256sum --check SHA256SUMS.txt`. → `tnt-library-0.2.0.jar: OK`. **Delivery integrity also verified:** the JAR's baked `pack-defaults.properties` `pack.sha1` (`d5115f30ed2dad4912344ed9127020be6f946572`) exactly equals the published `tnt-library-pack-0.2.0.zip` SHA-1, and `pack.url` points to the v0.2.0 asset — so a Java client fetches the exact published pack (now carrying the twins + smartbomb cubes) and its hash check passes.
